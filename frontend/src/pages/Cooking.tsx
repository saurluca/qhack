    ...
    try {
      ...
      try {
        ...
        const ingredientsResponse = await fetch(
          `/recipes/ingredients_${videoId}.json`,
        );
      ...
      try {
        ...
        const response = await fetch(
          `${BACKEND_URL}/extract-recipe?url=${encodeURIComponent(videoUrl)}`,
          {
            method: "GET",
        ...
        if (!response.ok) {
          throw new Error(
            `Failed to extract recipe: ${response.status} ${response.statusText}`,
          );
        }
        ...
        if (error instanceof DOMException && error.name === "AbortError") {
          ...
          throw new Error(
            "Network error: Unable to connect to the backend server. Make sure it is running at " +
              BACKEND_URL,
          );
        } else {
          throw error;
      ...
      setError(
        `Failed to extract recipe: ${err instanceof Error ? err.message : "Unknown error"}`,
      );
    } finally {
      setIsLoading(false);
    }
  ...
  const handleSelectRecipe = async (recipeId: string) => {
    try {
      ...
      try {
        ...
        const ingredientsResponse = await fetch(
          `/recipes/ingredients_${recipeId}.json`,
        );
          ...
          <div className="space-y-6">
            <div className="bg-white rounded-lg shadow p-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                {/* Ingredients */}
                <div>
                  <h3 className="font-semibold text-lg border-b border-gray-200 pb-2 mb-4">
                    Ingredients
                  </h3>
                  <ul className="space-y-2">
                    {recipeData.ingredients.map((ingredient, index) => (
                      <li key={index} className="flex items-center">
                        ...
                        />
                        <span
                          className={`ml-2 ${checkedIngredients[ingredient] ? "line-through text-gray-400" : ""}`}
                        >
                          {ingredient}
                        </span>
                      </li>
